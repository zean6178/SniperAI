...
const C = {
  bg1: '#FFFFFF', bg2: '#F5F3FF', bg3: '#EDE9FE',
  purple: '#8B5CF6', purpleVib: '#7C3AED',
  textW: '#1A1A2E', textSec: '#4A4A68', textTert: '#8888A0', textDis: '#B8B8D0',
...
function phone(x, y, content, label) {
  return `
    <g transform="translate(${x}, ${y})">
      <rect x="0" y="0" width="${W}" height="${H}" rx="44" fill="${C.bg1}" stroke="#D4D4E8" stroke-width="3"/>
...
function tabBar(active) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (isAct) items += `<circle cx="${cx}" cy="${y+22}" r="15" fill="${C.bg3}"/>`;
...
function homeContent() {
  return `
    ...
    <line x1="0" y1="92" x2="${W}" y2="92" stroke="${C.border}"/>
...
function portfolioContent() {
  const posCard = (s, e, m, p, pnl, profit) => {
    const col = profit ? C.success : C.danger;
    const pW = Math.min(100, Math.max(10, (m/p)*100));
    return `
    <rect x="20" y="${e}" width="${W-40}" height="100" rx="16" fill="${C.bg2}" stroke="${C.border}"/>
    <circle cx="52" cy="${e+36}" r="18" fill="${C.bg3}"/>
    <text x="52" y="${e+41}" text-anchor="middle" fill="${C.purple}" font-size="14" font-weight="700">${s[0]}</text>
    <text x="78" y="${e+31}" fill="${C.textW}" font-size="14" font-weight="600">${s}</text>
    <text x="78" y="${e+48}" fill="${C.textTert}" font-size="11">${profit ? '0.50 SOL · 8m' : '0.50 SOL · 22m'}</text>
    <text x="${W-56}" y="${e+36}" text-anchor="end" fill="${col}" font-size="20" font-weight="800">${m.toFixed(2)}x</text>
    <rect x="${W-100}" y="${e+44}" width="44" height="18" rx="9" fill="${col}" opacity="0.12"/>
    <text x="${W-78}" y="${e+57}" text-anchor="middle" fill="${col}" font-size="10" font-weight="600">${profit?'+':''}${pnl}%</text>
    <rect x="36" y="${e+74}" width="${W-96}" height="3" rx="2" fill="${C.bg3}"/>
    <rect x="36" y="${e+74}" width="${(W-96)*pW/100}" height="3" rx="2" fill="${col}"/>
    <text x="${W-56}" y="${e+80}" text-anchor="end" fill="${C.textDis}" font-size="9">Peak: ${p.toFixed(2)}x</text>`;
  };
  ...
  return `
    ...
    <rect x="36" y="162" width="160" height="24" rx="12" fill="${C.success}" opacity="0.12"/>
    ...
    ${posCard('PEPE2', 326, 2.1, 2.3, 110, true)}
    ${posCard('CHAD', 440, 0.8, 1.3, -20, false)}
    ${tabBar(1)}
  `;
}
...
function aiChatContent() {
  return `
    ...
    <rect x="260" y="484" width="40" height="20" rx="10" fill="${C.purple}" opacity="0.15"/>
    ...
    <rect x="260" y="526" width="40" height="20" rx="10" fill="${C.purple}" opacity="0.15"/>
...
function settingsContent() {
  const dataRow = (y, l, v) => `<text x="32" y="${y}" fill="${C.textSec}" font-size="12">${l}</text><text x="${W-48}" y="${y}" text-anchor="end" fill="${C.textW}" font-size="12" font-weight="600">${v}</text><line x1="32" y1="${y+8}" x2="${W-48}" y2="${y+8}" stroke="${C.border}"/>`;
  ...
  return `
    ...
    <rect x="32" y="186" width="200" height="22" rx="11" fill="${C.purple}" opacity="0.1" stroke="${C.purple}" stroke-opacity="0.3"/>
    <text x="42" y="201" fill="${C.purple}" font-size="10" font-weight="600">✦ Seeker Genesis Holder · 50% Off</text>
    ...
    <rect x="16" y="376" width="${W-32}" height="150" rx="16" fill="${C.bg2}" stroke="${C.border}"/>
    ...
    ${dataRow(476, 'Min Score for Alert', '75')}
    ${dataRow(504, 'Alert on Rug Detected', 'On')}
    <rect x="16" y="540" width="${W-32}" height="150" rx="16" fill="${C.bg2}" stroke="${C.border}"/>
    <text x="32" y="564" fill="${C.purple}" font-size="11" font-weight="700">RISK MANAGEMENT</text>
    <text x="32" y="590" fill="${C.textW}" font-size="13">Auto-Trade</text>
    <text x="32" y="605" fill="${C.textTert}" font-size="9">Auto execute on high scores</text>
    <rect x="${W-72}" y="580" width="40" height="22" rx="11" fill="${C.bg3}"/>
    <circle cx="${W-61}" cy="591" r="8" fill="${C.textTert}"/>
    <line x1="32" y1="618" x2="${W-48}" y2="618" stroke="${C.border}"/>
    ${dataRow(640, 'Buy Amount', '0.5 SOL')}
    ${dataRow(668, 'Stop Loss', '-40%')}
    ${tabBar(3)}
  `;
}
...
const GAP = 32;
const PAD = 24;
const totalW = (W + GAP) * 4 + PAD * 2 - GAP;
const totalH = H + 80;
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${totalW}" height="${totalH}" viewBox="0 0 ${totalW} ${totalH}">
  <rect width="${totalW}" height="${totalH}" fill="#F0EFF5"/>
...
const outPath = path.resolve(__dirname, 'sniperai-ui-preview-light.png');
await sharp(Buffer.from(svg)).png().toFile(outPath);
console.log('Light mode preview saved:', outPath);
